...
Plugin URI: http://multinc.com/wp/social-privacy/subscribe2-for-social-privacy/
Description:  Part of the Social Privacy set of plugins.  Based on the 3rd-party Subscribe2, this plugin allows registered and unregistered users to be notified via email whenever there is a new post that they are permitted to access.  Users can choose between different email contents and post categories.
Version: 1.1
Author: Justin
...
/*
This development is based on Subscribe2 version 4.9 by Matthew Robinson
*/
...
/* ===== WordPress menu registration ===== */
	...
	*/
	function admin_menu() {
		...
		$s2options = add_options_page(__('Social Subscribe2 Options', 'subscribe2'), __('Social Subscribe2', 'subscribe2'), "manage_options", __FILE__, array(&$this, 'options_menu'));
...
/* ===== mail handling ===== */
	...
	*/
	function publish($post = 0) {
		...
		// this is for new warning message setting
		$post_content = $post->post_content;
		if (is_callable(array('social_access_control', 'post_is_public'))) {
			if ('yes'==$this->subscribe2_options['show_warning_message'] && !social_access_control::post_is_public($post->ID))
				$post_content = "(".$this->subscribe2_options['warning_message'].")\n\r".$post->post_content;
		}
		$plaintext = $post_content;
		...
		// We want to have admin level control to limit the email content when sending out
		// admin now can control how much content subscribers can recieve on public posts and protected posts
		// (we assume public posts are posts that sit on the categories that can be viewed by any users, anonymous and logged in)
		// Eg. 	- When admin set to "full-post", all subscriber preferences will remain the same
		//		- When admin set to "excerpt', all subscribers with full-post preference will only receive excerpt, and so on.
		// NOTE: This can be set at Subscribe2 setting page
		
		// NOTICE: when we declare the "post_id" param, it means that all the users that don't have social_access_control will be excluded to the registered list
		
		// check if we have new social_access_control
		$post_id = $post->ID;
		if (is_callable(array('social_access_control', 'post_is_public'))) {
			...
			if (social_access_control::post_is_public($post_id)) {	// public posts
				if ('excerpt' == $this->subscribe2_options['public_email_type']) {
					$email_html = $excerpt;
					$email_plaintext = $excerpt;
					$email_excerpt = $excerpt;
				}
				elseif ('title' == $this->subscribe2_options['public_email_type']) {
					$email_html = '';
					$email_plaintext = '';
					$email_excerpt = '';
				}
				elseif ('non' == $this->subscribe2_options['public_email_type']) {
					$no_send = true;
				}
			}
			else {
				if ('excerpt' == $this->subscribe2_options['protected_email_type']) {
					$email_html = $excerpt;
					$email_plaintext = $excerpt;
					$email_excerpt = $excerpt;
				}
				elseif ('title' == $this->subscribe2_options['protected_email_type']) {
					$email_html = '';
					$email_plaintext = '';
					$email_excerpt = '';
				}
				elseif ('non' == $this->subscribe2_options['protected_email_type']) {
					$no_send = true;
				}
			}
		...
		// to adapt the above changes, notice the use of post_id param

		// first we send plaintext summary emails
		$body = str_replace("POST", $email_excerpt, $mailtext);
		$registered = $this->get_registered("cats=$post_cats_string&format=text&amount=excerpt&post_id=$post_id");
		if (empty($registered)) {
		...
		$body = str_replace("POST", strip_tags($email_plaintext), $mailtext);
		$recipients = $this->get_registered("cats=$post_cats_string&format=text&amount=post&post_id=$post_id");
		$this->mail($recipients, $subject, $body);
		// finally we send html full content emails
		$body = str_replace("\r\n", "<br />\r\n", $mailtext);
		$body = str_replace("POST", $email_html, $body);
		$recipients = $this->get_registered("cats=$post_cats_string&format=html&post_id=$post_id");
...
/* ===== Subscriber functions ===== */
	...
	*/
	/*
	Params added by Justin at Multinc
		post_id, filter
	*/
	function get_registered ($args = '') {
		...
		// Added by Justin Tran at Multinc
		if (!isset($r['post_id']))
			$r['post_id'] = '0';
		if (!isset($r['filter']))
			$r['filter'] = 'allowed';
		...
		// Added by Justin at Multinc
		// base on the post, we want to get the users that have social_access_control permission
		// first check if new social_access_control is there
		if (is_callable(array('social_access_control', 'post_should_be_hidden_to_user'))) {
			if ($r['post_id']!='0') {
				foreach ($result as $key => $user_id) {
					...
					$is_user_cannot_view = social_access_control::post_should_be_hidden_to_user((int)$r['post_id'], $target_user);
					// when filter = allowed, exclude any users that they don't have social_access_control permission
					if ($r['filter']=='allowed') {
						if ($is_user_cannot_view==true)
							unset($result[$key]);
					// when type = disallowed, only get the users that don't have social_access_control permission
					// in case we want to handle this kind of users in the future
					} else {
						if ($is_user_cannot_view==false)
							unset($result[$key]);
					}
				}
			}
	...
	function options_menu() {
		...
		if (isset($_POST['s2_admin'])) {
			...
			if ($_POST['reset']) {
				...
				//Added by Justin at Multinc
				//email levels for public & protected posts
				$this->subscribe2_options['protected_email_type'] = $_POST['protected_email_type'];
				$this->subscribe2_options['public_email_type'] = $_POST['public_email_type'];
				$this->subscribe2_options['show_warning_message'] = isset($_POST['show_warning_message']) ? $_POST['show_warning_message']:'no';
				$this->subscribe2_options['warning_message'] = $_POST['warning_message'];
				// original ...
		...
		//added by Justin at Multinc
		echo "<p>";
		echo "<h2>" . __('Email Content', 'subscribe2') . "</h2>\r\n";
		if (is_callable(array('social_access_control', 'post_is_public'))) {			
			echo "" . __('(Posts in public categories can be viewed by all users, anonymous or logged in)', 'subscribe2') . "<br /><br />\r\n";
			echo"<br />";
			echo __('Public categories', 'subscribe2') . ': ';
			echo "<input type=\"radio\" name=\"public_email_type\" value=\"full\"";
			if ('full' == $this->subscribe2_options['public_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Full post', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"public_email_type\" value=\"excerpt\"";
			if ('excerpt' == $this->subscribe2_options['public_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Excerpt', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"public_email_type\" value=\"title\"";
			if ('title' == $this->subscribe2_options['public_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Title only', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"public_email_type\" value=\"non\"";
			if ('non' == $this->subscribe2_options['public_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('No email', 'subscribe2') . "<br /><br />\r\n";
			echo __('Protected categories', 'subscribe2') . ': ';
			echo "<input type=\"radio\" name=\"protected_email_type\" value=\"full\"";
			if ('full' == $this->subscribe2_options['protected_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Full-post', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"protected_email_type\" value=\"excerpt\"";
			if ('excerpt' == $this->subscribe2_options['protected_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Excerpt', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"protected_email_type\" value=\"title\"";
			if ('title' == $this->subscribe2_options['protected_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('Title only', 'subscribe2') . " &nbsp;&nbsp;";
			echo "<input type=\"radio\" name=\"protected_email_type\" value=\"non\"";
			if ('non' == $this->subscribe2_options['protected_email_type']) {
				echo " checked=\"checked\" ";
			}
			echo " /> " . __('No email', 'subscribe2') . "<br /><br />\r\n";
		}
		else {
			echo "This setting is not enable until you have social_access_control activated\r\n";
		}
		echo "</p>";

		echo "<p>";
		echo "<h2>" . __('Warning message', 'subscribe2') . "</h2>\r\n";
		if (is_callable(array('social_access_control', 'post_is_public'))) {			
			print "<p><input name=\"show_warning_message\" type=\"checkbox\" value=\"yes\" " .
			('yes' == $this->subscribe2_options['show_warning_message'] ? " checked" : "") .
				"> ". __('Show a warning message in the email for protected posts.', 'subscribe2') ."</p>\n";
	
			print "<p><input name=warning_message type=text size=50" .
				" value=\"".$this->subscribe2_options['warning_message']."\" /></p>\n";
		}
		else {
			echo "This setting is not enable until you have social_access_control activated\r\n";
		}
